...
const handleIcons = (button: AccordionButton, isExpanded: boolean, isRotating = false): void => {
  const openIcon = button.querySelector<HTMLElement>("[data-dui-accordion-icon-open]");
  const closeIcon = button.querySelector<HTMLElement>("[data-dui-accordion-icon-close]");
  const rotatingIcon = button.querySelector<HTMLElement>("[data-dui-accordion-icon]");
...
export function toggleAccordion(event: Event): void {
  ...
  const parentElement = button.closest<AccordionContainer>("[data-dui-accordion-container]");
  ...
  if (targetID?.startsWith("#")) {
    ...
    if (mode === "exclusive" && parentElement) {
      parentElement.querySelectorAll<AccordionButton>("[data-dui-accordion-toggle]").forEach(otherButton => {
...
export function toggleAccordionById(targetId: string): void {
  ...
  const toggleButton = document.querySelector<AccordionButton>(
    `[data-dui-accordion-target="${targetId}"]`
  );
  ...
  const parentElement = toggleButton.closest<AccordionContainer>("[data-dui-accordion-container]");
  const mode = parentElement?.dataset.duiAccordionMode;
...
export function initAccordion(): void {
  document.querySelectorAll<AccordionButton>("[data-dui-accordion-toggle]").forEach((button) => {
